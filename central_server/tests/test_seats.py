...
from central_server.config import HEARTBEAT_TTL, SEAT_GRACE
...
def test_second_dispatch_to_another_server_is_refused(session_db):
    ...
    assert db.take_seat('alice', first, HEARTBEAT_TTL)
    assert not db.take_seat('alice', second, HEARTBEAT_TTL)


def test_redispatch_to_the_same_server_is_allowed(session_db):
    server_id = _add_server(session_db)

    assert db.take_seat('alice', server_id, HEARTBEAT_TTL)
    assert db.take_seat('alice', server_id, HEARTBEAT_TTL)


def test_seat_of_a_dead_server_is_taken_over(session_db):
    dead = _add_server(session_db, seen_ago=HEARTBEAT_TTL + 5)
    alive = _add_server(session_db)

    assert db.take_seat('alice', dead, HEARTBEAT_TTL)
    # The owner is gone, so the player is not locked out of a new table.
    assert db.take_seat('alice', alive, HEARTBEAT_TTL)


def test_heartbeat_releases_the_seat_of_a_player_who_left(session_db):
    server_id = _add_server(session_db)
    db.take_seat('alice', server_id, HEARTBEAT_TTL)
    ...
    assert db.take_seat('alice', other, HEARTBEAT_TTL)
...
def test_heartbeat_spares_a_seat_the_player_has_not_reached_yet(session_db):
    ...
    assert not db.take_seat('alice', other, HEARTBEAT_TTL)
