...
def _add_server(session_db, seen_ago=0, capacity=10):
...
def test_refuse_second_dispatch_to_another_server(session_db):
    ...
    db.take_seat(USERNAME_1, first)
    with pytest.raises(ValueError):
        db.take_seat(USERNAME_1, second)
...
def test_takeover_seat_from_dead_server(session_db):
    ...
    db.take_seat(USERNAME_1, dead)
    db.take_seat(USERNAME_1, alive)
...
def test_refuse_seat_on_full_server(session_db):
    server = _add_server(session_db, capacity=1)
    db.take_seat(USERNAME_1, server)

    with pytest.raises(db.ServerFull):
        db.take_seat(USERNAME_2, server)


def test_full_server_still_reseats_its_own_player(session_db):
    server = _add_server(session_db, capacity=1)
    db.take_seat(USERNAME_1, server)
