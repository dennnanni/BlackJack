...
Defined twice, and the two must agree: `database/db_create.sql` (reference SQL) and
the SQLAlchemy models in `central_server/db.py` (what the app actually uses —
`init_db()` runs `create_all` at startup, so you never need to run the SQL by hand).
